...
class RecEraser_WMF(object):
    ...
    def train_single_model(self, local_num):
        ...
        pos_item = tf.nn.embedding_lookup(
            self.weights["item_embedding"][:, local_num], self.input_ur
        )
        ...
        local_iw = tf.nn.embedding_lookup(
            self.weights["item_embedding"][:, local_num], self.C_I[local_num]
        )
    ...
    def attention_based_agg(self, embs, flag):
        if flag == 0:
            ...
            embs_w = tf.div(embs_w, tf.reduce_sum(embs_w, 1, keepdims=True))
        else:
            embs_w = tf.exp(
                tf.einsum(
                    "abc,ck->abk",
    ...
    def _create_loss(self, user, items, pos_r):
        loss = self.weight1 * tf.reduce_sum(
            tf.einsum("ab,ac->bc", items, items) * tf.einsum("ab,ac->bc", user, user)
        )
            ...
            for epoch in range(args.epoch):
                ...
                if (epoch + 1) % 10 != 0:
                    if args.verbose > 0 and epoch % args.verbose == 0:
                        perf_str = "[local_model %d] Epoch %d [%.1fs]: train==[%.5f]" % (
                            i,
                            epoch,
                            time() - t1,
                            loss,
                ...
                if args.verbose > 0:
                    perf_str = (
                        "[local_model %d]  Epoch %d [%.1fs + %.1fs]:, recall=[%.5f, %.5f], "
