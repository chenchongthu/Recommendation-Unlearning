...
class RecEraser_LightGCN(object):
    ...
    def train_single_model(self, local_num):
        ...
        u_g_embeddings_pre = tf.nn.embedding_lookup(
            self.weights["user_embedding"][:, local_num], self.users
        )
        pos_i_g_embeddings_pre = tf.nn.embedding_lookup(
            self.weights["item_embedding"][:, local_num], self.pos_items
        )
        neg_i_g_embeddings_pre = tf.nn.embedding_lookup(
        ...
        batch_ratings = tf.matmul(
            u_g_embeddings, pos_i_g_embeddings, transpose_a=False, transpose_b=True
        )
    ...
    def train_agg_model(self):
        ...
        u_es = (
            tf.einsum("abc,bcd->abd", u_g_embeddings, self.weights["trans_W"])
            + self.weights["trans_B"]
        )
        pos_i_es = (
            tf.einsum("abc,bcd->abd", pos_i_g_embeddings, self.weights["trans_W"])
            + self.weights["trans_B"]
        )
        neg_i_es = (
            tf.einsum("abc,bcd->abd", neg_i_g_embeddings, self.weights["trans_W"])
            + self.weights["trans_B"]
        )
    ...
    def train_agg_model2(self):
        ...
        u_es = (
            tf.einsum("abc,bcd->abd", user_local_embs, self.weights["trans_W"])
            + self.weights["trans_B"]
        )
        i_es = (
            tf.einsum("abc,bcd->abd", item_local_embs, self.weights["trans_W"])
            + self.weights["trans_B"]
        )
    ...
    def _create_lightgcn_embed_local(self, local_num):
        ...
        ego_embeddings = tf.concat(
            [
                self.weights["user_embedding"][:, local_num],
                self.weights["item_embedding"][:, local_num],
            ],
            axis=0,
        )
        all_embeddings = [ego_embeddings]
    ...
    def _dropout_sparse(self, X, keep_prob, n_nonzero_elems):
        ...
        return pre_out * tf.div(1.0, keep_prob)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epoch_agg):
        ...
        for idx in range(n_batch):
            ...
            _, batch_loss, batch_mf_loss, u_w = sess.run(
                [model.opt_agg, model.loss_agg, model.mf_loss_agg, model.u_w],
                feed_dict={
                    model.users: users,
                    model.pos_items: pos_items,
                    model.neg_items: neg_items,
        ...
        if args.verbose > 0:
            perf_str = (
                "Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f ], recall=[%.5f, %.5f, %.5f], "
