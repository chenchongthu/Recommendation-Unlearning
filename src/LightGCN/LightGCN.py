...
class LightGCN(object):
    ...
    def create_model_str(self):
        log_dir = (
            "/" + self.alg_type + "/layers_" + str(self.n_layers) + "/dim_" + str(self.emb_dim)
        )
    ...
    def _init_weights(self):
        ...
        for k in range(self.n_layers):
            all_weights["W_gc_%d" % k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k + 1]]),
                name="W_gc_%d" % k,
            )
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epoch):
        ...
        loss_test, mf_loss_test, emb_loss_test, reg_loss_test = 0.0, 0.0, 0.0, 0.0
